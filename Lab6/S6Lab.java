...
public class S6Lab {
    ...
    private static final int x1min = -25;
    private static final int x1max = -5;
    private static final int x2min = -15;
    private static final int x2max = 35;
    private static final int x3min = -5;
    private static final int x3max = 60;
    ...
    private static void generateMatrix() {
        ...
        for (int i = 0; i < mainMatrix.length; i++) {
            for (int j = 10; j < mainMatrix[i].length-1; j++) {
                mainMatrix[i][j] = (int)(Math.random()*10) - 5 + 1    + 5.4 + 0.4*mainMatrix[i][0] + 1.5*mainMatrix[i][1] + 9.3*mainMatrix[i][2] + 9.5*mainMatrix[i][7] + 0.3*mainMatrix[i][8] + 2.2*mainMatrix[i][9] + 1.1*mainMatrix[i][3] + 0.7*mainMatrix[i][4] + 9.3*mainMatrix[i][5] + 3.7*mainMatrix[i][6];
            }
        }

        for(int i = 0; i < mainMatrix.length; i++){
            for(int j = 10; j < mainMatrix[i].length -1 ; j++){
                mainMatrix[i][mainMatrix[i].length -1] += mainMatrix[i][j]/(m+1);
            }
        }
    }
    ...
    private static void getMatrix() {
        ...
        System.out.print("        X1        X2        X3       X1X2      X1X3     X2X3    X1X2X3      X1^2      X2^2      X3^2  //   Y1...Yav\n");
    ...
    private static Matrix generateStandartMatrix(){
        double m00 = 0,m10 = 0,m20 = 0,m30 = 0 ,m40 = 0 ,m50 = 0,m60 = 0 ,m70 = 0 ,m80 = 0,m90 = 0,m100=0,m01 = 0,m11 = 0,m21 = 0,m31 = 0
                ,m41 = 0,m51 = 0,m61 = 0,m71 = 0,m81 = 0,m91= 0,m101 = 0,m02 = 0,m12 = 0,m22 = 0,m32 = 0,
                m42= 0 ,m52 = 0,m62 = 0,m72 = 0,m82 = 0,m92 = 0, m102 = 0,m03 = 0,m13 = 0,m23 = 0,m33 = 0,m43 = 0,m53 = 0,m63 = 0 ,m73 = 0,
                m83 = 0, m93  =0, m103 = 0, m04 = 0,m14 = 0,m24 = 0
                ,m34 = 0,m44 = 0,m54 = 0,m64 = 0,m74 =0 ,m84 = 0, m94 = 0, m104 = 0,m05 = 0 ,m15 = 0,m25 = 0,m35 =0,m45 =0,m55 =0,m65 =0,m75 =0,
                ...
                m87=0,m97=0,m107=0,m08=0,m18=0,m28=0,m38=0,m48=0,m58=0,m68=0,m78=0,m88=0,m98=0,m108=0,m09=0,m19=0,m29=0,m39=0,m49=0,m59=0,m69=0,m79=0
                ,m89=0,m99=0,m109=0,m010=0,m110=0,m210=0,m310=0,m410=0,m510=0,m610=0,m710=0,m810=0,m910=0,m1010=0;
    ...
    private static void regressionEquation(){
        ...
        System.out.printf("y = %.2f + %.2fx1 + %.2fx2 + %.2fx3 + %.2fx1x2 + %.2fx1x3 + %.2fx2x3 + %.2f x1x2x3 + %.2f x1^2 + %.2f x2^2 + %.2f x3^2\n",b0,b1,b2,b3,b12,b13,b23,b123,b11,b22,b33);
        System.out.println("f(x1,x2,x3) = 5,0 + 4,7*x1 + 3,6*x2 + 6,4*x3 + 6,8*x1*x1 + 0,3*x2*x2 + 5,3*x3*x3 + 3,2*x1*x2 + 0,9*x1*x3 + 2,7*x2*x3 + 0,1*x1*x2*x3");
        System.out.println("Зоряне плече   l = k^(1/2) тому що всі зоряні точки знаходяться на однаковій відстані від початку експерименту\n l = 3^(1/2) = 1.73");
    }
    ...
    private static void critKohren(){
        ...
        System.out.printf("f1 = %.2f\n" +
                "Gp = %.2f\n",f1,Gp);
        if(Gp < 0.5157) System.out.println("Dispersion is homogeneous!");
        else {
    ...
    private static void critStudent(){
        ...
        if(B[1]/Sbs > 2.12) System.out.print("");
        if(B[2]/Sbs < 2.12) System.out.print("");
        if(B[3]/Sbs > 2.12) System.out.print("");
        if(B[4]/Sbs < 2.12) System.out.print("");
        if(B[5]/Sbs < 2.12) System.out.println("b1b3 is unnecessary");
        if(B[6]/Sbs < 2.12) System.out.print("");
        if(B[7]/Sbs < 2.12) System.out.println("b1b2b3 is unnecessary");
        if(B[8]/Sbs < 2.12) System.out.print("");
    ...
    private static void critFisher() {
        ...
        System.out.println("f4 = N - d = 8-3 = 5\n" +
                ...
                "Ft = 2.9");
        if (Fp > 4.1) System.out.println("Regression equation is inadequate to the original!");
        else System.out.println("Regression equation is adequate to the original!");
    }
